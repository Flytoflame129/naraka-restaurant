...
const requirements = [
  ...
  ["repeatable experience controller", controller.includes("cleanupPreviousInitialization?.(true)") && controller.includes("controller.abort()") && controller.includes('document.addEventListener("astro:page-load", initSiteExperience)')],
  ["navigation capability survives router swaps", controller.includes('root.classList.add("js")') && controller.includes('document.addEventListener("astro:before-swap"') && controller.includes('document.addEventListener("astro:after-swap", initSiteExperience)') && controller.includes('if (!preserveNavigation) root.classList.remove("site-experience-ready")')],
